...
<li class="<?php if ($index === 1 || !$handler->isCoreFile($file)) echo 'application'; ?> call-stack-item"
    ...
    <?php if (!empty($lines)): ?>
        <div class="code-wrap">
            ...
            <div class="code">
                <?php for ($i = $begin; $i <= $end; ++$i): ?><a href="#" class="lines-item" onclick="_goToEditorCodeLine('<?= str_replace('\\', '\\\\', $handler->htmlEncode($file))?>', '<?= (int) $i + 1 ?>'); return false;"><?= (int) $i + 1 ?></a><?php endfor; ?>
                <pre><?php
