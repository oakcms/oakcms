                ...
                <tr>
                    ...
                    <td width="120" class="text-right">
                        <div class="dropdown actions">
                            <i id="dropdownMenu<?= $cat->category_id ?>" data-toggle="dropdown" aria-expanded="true" title="<?= Yii::t('content', 'Actions') ?>" class="glyphicon glyphicon-menu-hamburger"></i>
                            <ul class="dropdown-menu dropdown-menu-right" role="menu" aria-labelledby="dropdownMenu<?= $cat->category_id ?>">
                                <li>
                                    <a href="<?= Url::to([$baseUrl . '/a/edit', 'id' => $cat->category_id]) ?>"><i class="glyphicon glyphicon-pencil font-12"></i> <?= Yii::t('content', 'Edit') ?>
                                    </a></li>
                                <li>
                                    <a href="<?= Url::to([$baseUrl . '/a/create', 'parent' => $cat->category_id]) ?>"><i class="glyphicon glyphicon-plus font-12"></i> <?= Yii::t('content', 'Add subcategory') ?>
                                    </a></li>
                                <li role="presentation" class="divider"></li>
                                <li>
                                    <a href="<?= Url::to([$baseUrl . '/a/up', 'id' => $cat->category_id]) ?>"><i class="glyphicon glyphicon-arrow-up font-12"></i> <?= Yii::t('content', 'Move up') ?>
                                    </a></li>
                                <li>
                                    <a href="<?= Url::to([$baseUrl . '/a/down', 'id' => $cat->category_id]) ?>"><i class="glyphicon glyphicon-arrow-down font-12"></i> <?= Yii::t('content', 'Move down') ?>
                                    </a></li>
                                <li role="presentation" class="divider"></li>
                                <?php if($cat->status == CategoryModel::STATUS_ON) :?>
                                    <li>
                                        <a href="<?= Url::to([$baseUrl . '/a/off', 'id' => $cat->category_id]) ?>" title="<?= Yii::t('content', 'Turn Off') ?>'"><i class="glyphicon glyphicon-eye-close font-12"></i> <?= Yii::t('content', 'Turn Off') ?>
                                        </a></li>
                                <?php else : ?>
                                    <li>
                                        <a href="<?= Url::to([$baseUrl . '/a/on', 'id' => $cat->category_id]) ?>" title="<?= Yii::t('content', 'Turn On') ?>"><i class="glyphicon glyphicon-eye-open font-12"></i> <?= Yii::t('content', 'Turn On') ?>
                                        </a></li>
                                <?php endif; ?>
                                <li>
                                    <a href="<?= Url::to([$baseUrl . '/a/delete', 'id' => $cat->category_id]) ?>" class="confirm-delete" data-reload="1" title="<?= Yii::t('content', 'Delete item') ?>"><i class="glyphicon glyphicon-remove font-12"></i> <?= Yii::t('content', 'Delete') ?>
                                    </a></li>
                            </ul>
                        </div>
                    </td>
