...
oak.createorder = {
    init: function() {
		...
		$(document).on("promocodeEnter", function(e, code) {
			if($(".order-create-container form input[name='Order[user_id]']").val() == '') {
				oak.createorder.chooseUser(code);
			}
		});
    ...
    updateCartUrl: '',
    updateCart: function() {
        $.post(oak.createorder.updateCartUrl, {},
            function(json) {
                $('.oak-cart-block').replaceWith(json.cart);
                $('.total').html(json.total);
                $('.oak-cart-count').html(json.count);
            }, "json");
